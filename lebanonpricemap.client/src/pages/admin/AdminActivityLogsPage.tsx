    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8">
        ...
        <div className="space-y-3 mt-8">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-20 rounded-xl bg-bg-muted animate-pulse" />
          ))}
        </div>
      </motion.div>
    );
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8">
      ...
      <div className="relative pl-8">
        ...
        <div className="flex flex-col gap-4">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-muted">by {log.performedByName ?? 'System'}</p>
                  </div>
                </div>
          ...
          {logs.length === 0 && (
            <p className="text-sm text-text-muted text-center py-16">No audit logs found.</p>
          )}
        </div>
      </div>
    </motion.div>
