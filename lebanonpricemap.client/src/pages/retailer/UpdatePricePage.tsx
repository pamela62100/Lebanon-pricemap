...
export function UpdatePricePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!priceValue || !storeId || !product.id) return;
    setIsSaving(true);
    try {
      // Try to update existing catalog item, or create new one
      await catalogApi.create({
        storeId,
        productId: product.id,
        officialPriceLbp: Number(priceValue),
        isPromotion: isPromo,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto flex flex-col gap-8">
      <div className="flex items-center justify-between pb-6 border-b border-border-soft">
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={handleSubmit} disabled={isSaving} className="h-11 px-6 rounded-xl font-bold bg-primary text-white hover:bg-primary-hover transition-all disabled:opacity-60">
        ...
        <div className="flex-1 max-w-xl">
          <form id="price-form" onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="bg-bg-surface border border-border-soft p-6 py-8 rounded-3xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)]">
              ...
              <LBPInput value={priceValue} onChange={(val) => setPriceValue(val)} placeholder="0" autoFocus />
            </div>
            ...
            <div className={cn(
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className={cn('w-14 h-8 rounded-full transition-colors relative flex items-center px-1', isPromo ? 'bg-primary' : 'bg-border-primary')}
                >
                  <motion.div layout className="w-6 h-6 rounded-full bg-white shadow-card" animate={{ x: isPromo ? 24 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                </button>
              </div>
            ...
            <div className="pointer-events-none scale-105 origin-top-left xl:scale-110">
              <PriceResultCard
                ...
                entry={{
                  ...
                  product: product,
                  store: {
                    id: storeId || 'mine', name: 'My Store', status: 'verified', trustScore: 99,
                    chain: null, city: 'Beirut', district: 'Achrafieh', latitude: 0, longitude: 0,
                    isVerifiedRetailer: true, ownerId: 'me', region: 'Beirut', internalRateLbp: 90000, powerStatus: 'stable'
                  }
                }}
              />
            </div>
